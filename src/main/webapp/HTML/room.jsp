...
<link rel="stylesheet" href="<c:url value="/assets/css/room.css"/>">
<link rel="stylesheet"
...
<body>
	...
	<div id="app" class="app__checkin">
		<div id="header" class="app__header">
			...
			<c:if test="${sessionScope.error_lg!=null}">
				<script>
						console.log("dmm")
						swal("Lỗi!", "Vui lòng đăng nhập trước khi thực hiện thao tác!", "error");
						${sessionScope.error_lg=null}
				</script>
			</c:if>
			<c:if test="${sessionScope.acc!=null}">
		...
		<div class="app__body">
			<div class="info-checkin">
				...
				<div id="rooms_infomation">
					<c:forEach items="${listRooms}" var="o" varStatus="loop">
						<div class="room">
							...
							<div class="room-inf">
								...
								<h2>
									Số phòng còn trống: <span class="max_available_num">${o.getAvailable_Room()}</span>
								</h2>
								<div style="width: 100%;" class="content-input-date-select">
									<div class="input-date-wrap input-date-checkin flatpickr">
										<i class="far fa-calendar-alt"></i> <input class="dateInOut"
											id="SelectDate" placeholder="Ngày Nhận phòng" value="">

									</div>
									<div class="input-date-wrap input-date-checkout flatpickr">
										<i class="far fa-calendar-alt"></i> <input class="dateInOut"
											id="SelectDate" placeholder="Ngày Trả phòng" value="">
									</div>
								</div>
							...
							<div class="option_btn">
								<form action="AddtoCart" method="post">
									<div style="display: none;">
										<input name="DayInAddToCart" class="DayInAddToCart" value="">
										<input name="DayOutAddToCart" class="DayOutAddToCart" value="">
										<input name="totalRoom" class="totalRoom" value=""> <input
											name="totalAdults" class="totalAdults" value=""> <input
											name="totalChildren" class="totalChildren" value="">
										<input name="Room_id" class="Room_id"
											value="${o.getRoom_id() }">
									</div>

									<button class="room-btn padding-12 blue-outline-button"
										type="submit" onclick=dayIn(${loop.index})>Thêm vào
										giỏ</button>
								</form>

								<form action="BookNow" method="post">
									<div style="display: none;">
										<input name="DayInAddToCart2" class="DayInAddToCart2" value="">
										<input name="DayOutAddToCart2" class="DayOutAddToCart2"
											value=""> <input name="totalRoom2" class="totalRoom2"
											value=""> <input name="totalAdults2"
											class="totalAdults2" value=""> <input
											name="totalChildren2" class="totalChildren2" value="">
										<input name="Room_id" class="Room_id"
											value="${o.getRoom_id() }"> <input name="Hotel_id"
											value="${o.getHotel_id()}">
									</div>

									<button class="room-btn blue-button  padding-12" type="submit"
										onclick=dayIn(${loop.index})>
	...
	function dateFormat(inputDate) {
		...
		function dayIn(i) {
			var dayin = document.getElementsByClassName("dateInOut")[i*2].value;
			document.getElementsByClassName("DayInAddToCart")[i].value = dateFormat(dayin);
			document.getElementsByClassName("DayInAddToCart2")[i].value = dateFormat(dayin);
			var dayOut = document.getElementsByClassName("dateInOut")[(i*2)+1].value;
			document.getElementsByClassName("DayOutAddToCart")[i].value = dateFormat(dayOut);
			document.getElementsByClassName("DayOutAddToCart2")[i].value = dateFormat(dayOut);
			var totalRoom = document.getElementsByClassName("rooms")[i].innerHTML;
			document.getElementsByClassName("totalRoom")[i].value = totalRoom;
			document.getElementsByClassName("totalRoom2")[i].value = totalRoom;
			var totalAdults = document.getElementsByClassName("adults")[i].innerHTML;
			document.getElementsByClassName("totalAdults")[i].value = totalAdults;
			document.getElementsByClassName("totalAdults2")[i].value = totalAdults;
			var totalChildren = document.getElementsByClassName("children")[i].innerHTML;
			document.getElementsByClassName("totalChildren")[i].value = totalChildren;
			document.getElementsByClassName("totalChildren2")[i].value = totalChildren;
			
// 			var totalChildren = document.getElementsByClassName("children")[i].value;
// 			document.getElementsByClassName("totalChildren")[i].value = dayOut;
			//return dayin;
		}
