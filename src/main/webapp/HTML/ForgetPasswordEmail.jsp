...
<body>
		...
		<div class="app__body">
			<div class="app__body-card login-card">
				<h1 class="app_body-header">
					Quên mật khẩu <span style="color: rgb(207, 32, 97);">Email</span>
				</h1>
				<p>Vui lòng nhập đúng Mail để có được gửi mã tạo lại mật khẩu</p>
				...
				<div class="app__body-card-form">
					...
					<c:if test="${emailCheck==null}">
						<form action="checkemail" method="post"
							...
							<div class="app__body-input-email">
								<label for="email-input">Email</label> <input name="email_login"
							...
							<button type="submit" id="change-pass"
								class="app__body-input-login-btn">Kiểm tra Email</button>
						</form>
					</c:if>

					<c:if test="${codeCheck!=null}">
						<form action="sendcode" method="post" id="change-pass-code-check"
							action="" class="app__body-input">
							<div id="code-wrap"
								class="app__body-input-password app__body-input-code">
								<span> <label for="code">Nhập mã xác thực được
										gửi về mail</label> <input name="code_input" type="text" autocomplete="off" id="code"
									class="app__body-input-text" placeholder="Sent code" required>
								</span>
									<span style="align-self: flex-end; height: 100%;"> <a
					...
					<c:if test="${resetPass!=null}">
						<form id="change-pass-form" action="resetpasswordbymail"
							method="post" class="app__body-input">
							<div class="app__body-input-password">
								<label for="password">Mật khẩu Mới</label> <input
									name="password-reset" type="password" id="password"
									class="app__body-input-text" placeholder="Mật khẩu mới"
									required>
							</div>

							<div
								class="app__body-input-password app__body-input-password-repeat">
								<label for="password-repeat">Nhập lại Mật khẩu</label> <input
									name="repassword-reset" type="password" id="password-repeat"
							...
							<c:if test="${error3!=null}">
								<div id="change-pass-error" class="login-error">
									<i class="fa-sharp fa-solid fa-circle-exclamation"></i>
									<p class="error">Mật khẩu không trùng khớp</p>
								</div>
							</c:if>


							<button id="change-pass" type="submit"
								class="app__body-input-login-btn">Đổi mật khẩu</button>
							<!-- </div> -->
						</form>
					</c:if>
				...
				<div class="app__body-policy">
					<p>
						Khi đăng nhập, tôi đồng ý với các <a href="">Điều khoản sử
							dụng</a> và <a href="">Chính sách bảo mật</a> của LHT Travel Company.
					</p>
				</div>
			</div>
	...
	<%-- 	<script src="<c:url value="/script/emptyText.js"/>"></script> --%>
	<!-- 	<script>
